...
const RECIPE_COMMENT_QUERY_LIMIT = 5;
...
export const getComments = async (pageNo: number, recipe_id: number) => {
    try {
        ...
        const updated_recipe_comments = await Promise.all(with_user_comments.map(async com => {
            return {...com, user: {...com.user, user_image: com.user ? await getFile(com.user.user_image) : ''}, created_at: new Date(com.created_at).toDateString()}
        }))
