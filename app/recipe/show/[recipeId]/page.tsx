import { getRecipeData, getRecipeTitle, isLikedExist } from "@/action/recipe";
...
export default async function ShowRecipe({
  ...
  const [ret, decryptedSession] = await Promise.all([
    await getRecipeData(Number(recipeId)),
    await getDecryptedSession(),
  ]);

  const user_details = decryptedSession
    ? await getUserDetails(decryptedSession.user.user_id)
    : undefined;
  const recipe_data = ret.body;

  if (recipe_data === undefined || recipe_data.user === undefined)
    redirect("/");

  const recipeIngredients = recipe_data.recipe_ingredients.map(
    (ingr) =>
      `${ingr.recipe_ingredients_name} ${ingr.recipe_ingredients_amount}`
  );
  const recipeInstructions = recipe_data.recipe_instructions.map(
    (instr) => `${instr.recipe_instructions_text}`
  );
  const recipe_images = recipe_data.recipe_images.map(
  ...
  const likeStatus = await isLikedExist(
    recipe_data.recipe_id,
    decryptedSession ? decryptedSession.user.user_id : -1
  );
  ...
  const structuredData = {
    ...
    image:
      recipe_data.recipe_images.length > 0
        ? recipe_data.recipe_images[0].recipe_image
        : "",
    author: {
      "@type": "Person",
      name: recipe_data.user.user_codename,
    },
    recipeIngredient: recipe_data.recipe_ingredients.map(
      (i) => `${i.recipe_ingredients_name} ${i.recipe_ingredients_amount}`
    ),
    recipeInstructions: recipe_data.recipe_instructions.map(
      (i) => `${i.recipe_instructions_text}`
    ),
  };
  ...
  return (
    <section className="flex justify-center flex-col items-center">
      ...
      <div style={{ maxWidth: MAXWSIZE }} className="w-full relative">
        ...
        />
        <div
          className={`tags-likes flex justify-between items-center px-[5px] py-[5px]`}
        >
          <div className={`w-[60%] flex gap-[5px] flex-wrap items-center `}>
            {recipe_data.recipe_age_tag !== "" ? (
              <span
                className={`bg-[#523636] self-center flex justify-center items-center text-white py-[2px] px-[7px] rounded-[5px] text-[${CardTagSize}]`}
              >
                {recipe_data.recipe_age_tag}
              </span>
            ) : null}
          ...
          <div className="flex gap-[5px] items-center">
            {isLoggedIn && (
              <LikeRecipe
                ...
                total_likes={recipe_data.total_likes}
      ...
      <div
        style={{ maxWidth: MAXWSIZE }}
        className="w-full content-container p-[20px] flex flex-col justify-center gap-[30px]"
      >
        ...
        <div className="recipe-description w-full flex justify-start items-center self-center">
          <span className="leading-snug text-[13px] whitespace-pre-line">
            {recipe_data.recipe_description}
          </span>
        </div>
        <div className="recipe-ingredients w-full flex flex-col justify-start items-start gap-[10px]">
          <h1
            style={{ fontSize: fontSize.l4 }}
            className="font-semibold text-[#523636]"
          >
            材料
          </h1>
          <ul
            className={`flex flex-wrap w-[100%] ${
              recipeIngredients.length > 5 ? "flex-row" : "flex-col"
            }`}
          >
              ...
              return (
                <>
                  <li
                    key={idx}
                    className={`${
                      recipeIngredients.length > 5 ? "basis-1/2" : "basis-1"
        ...
        <div className="w-full relative">
          ...
          >
            ...
            <span className="text-[10px]">
              {Number(recipe_data.recipe_rating_data.avgRating).toFixed(1)} (
              {recipe_data.recipe_rating_data.totalRating})
            </span>
          </h1>
        ...
        <CommentSection
          ...
          recipe_id={recipe_data.recipe_id}
          total_comments={recipe_data.recipe_rating_data.totalRating}
        />
        <CommentForm
