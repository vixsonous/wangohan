...
export default function RecipeOptions({ recipe_id }: { recipe_id: number }) {
  ...
  const deleteModal = () => dispatch(showModal(modalIds.recipeoptionsModal));
  ...
  const openOptions = useCallback(
    () => setState((prev) => ({ ...prev, displayMenu: !state.displayMenu })),
    [state.displayMenu]
  );
    ...
    <>
      <div className="absolute z-[5] right-0">
        ...
        <motion.div
          key={1}
          initial={"closed"}
          animate={state.displayMenu ? "open" : "closed"}
          exit={"closed"}
          variants={menuVariants}
          className={`absolute top-8 right-12 origin-top-right`}
        >
          <div className="flex flex-col relative z-20 p-[15px] gap-[5px]">
            <button className="flex gap-[5px] items-center justify-center z-20">
              ...
              <Link href={`/recipe/edit/${recipe_id}`}>
                <h1
                  style={{ fontSize: fontSize.l2 }}
                  className="whitespace-nowrap text-[#FFFAF0]"
                >
                  編集する
                </h1>
              </Link>
            ...
            <button
              onClick={deleteModal}
              className="flex gap-[5px] items-center justify-center z-20"
            >
              <FontAwesomeIcon
                icon={faTrash}
                color={"#FFFAF0"}
        ...
        <div className="bg-secondary-bg flex flex-col gap-4 z-[999] py-4 px-8 rounded-lg">
          ...
          <div className="flex justify-end gap-2">
            <button
              ...
              className="w-32 text-center py-2 px-4 bg-gray-200 rounded-lg"
            >
              キャンセル
            </button>
            <button
              ...
              className={`w-32 justify-center py-2 px-4 bg-red-500 text-white rounded-lg flex gap-2 items-center ${
                state.isDeleting ? "opacity-50" : ""
              }`}
            >
              {state.isDeleting && (
