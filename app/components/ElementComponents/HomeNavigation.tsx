...
export default function HomeNavigation() {
  ...
  return (
    <div className="flex text-sm md:text-base gap-4 mt-10 flex-wrap justify-center">
      ...
      <span className="select-none">|</span>
      <Link href={'/recipe/list/1'} className="text-primary-text font-semibold hover:text-[#9ba3af]">レシピ図鑑</Link>
      <span className="select-none hidden md:flex">|</span>
      <Link href={'/columns'} className="text-primary-text font-semibold hover:text-[#9ba3af]">犬と食に関するコラム</Link>
      <span className="select-none">|</span>
      <Link href={isLoggedIn ? `/user/settings/${user.user_id}?=#register-pet` : '/login'} className="text-primary-text font-semibold hover:text-[#9ba3af]">愛犬登録</Link>
    </div>
  )
} 