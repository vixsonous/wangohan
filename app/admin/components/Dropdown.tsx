...
const IconSize = 20;
export default memo(function Dropdown({ children, openIcon = <Button><List size={IconSize} /></Button>, closeIcon = <Button><X size={IconSize} /></Button>, closeOnClick = true, className = '' }: { children: React.ReactNode, openIcon?: React.ReactNode, closeIcon?: React.ReactNode, closeOnClick?: boolean, className?: string }) {
  ...
  const triggerOnClick = useCallback((e: React.MouseEvent<HTMLElement>) => {
    ...
    if (t) {
      setState(prev => ({ ...prev, overflow: t.getBoundingClientRect().left < 50 }))
    }
  }, [state.open]);
